...
export const useDistributorStore = create<DistributorStoreState>()(
  persist(
    (set, get) => ({
      ...
      // Demo PIN exists only in development with VITE_ENABLE_DEMO_UNLOCK=true — never in production builds
      adminPin: DEMO_UNLOCK_ENABLED ? DEMO_PIN : '',
      ...
      verifyPin: (pin) => {
        ...
        const state = get();
        // No hardcoded PIN bypasses — adminPin is empty in production
        if (clean.length > 0 && clean === state.adminPin) {
