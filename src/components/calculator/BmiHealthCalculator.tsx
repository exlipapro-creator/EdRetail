...
export const BmiHealthCalculator: React.FC<BmiHealthCalculatorProps> = ({
  ...
  const bmiCategory = useMemo(() => {
    if (bmi < 18.5) {
      return {
        ...
        riskLevelSw: 'Kinga dhaifu ya mwili & uchovu',
        riskLevelEn: 'Low immunity & nutritional deficiency risk',
        level: 1,
      };
    }
    if (bmi >= 18.5 && bmi <= 24.9) {
      return {
        ...
        riskLevelEn: 'Excellent metabolic status. Maintain with regular detox and vitality drinks.',
        level: 2,
      };
    }
    if (bmi >= 25 && bmi <= 29.9) {
      return {
        ...
        riskLevelSw: 'Kitambi & msukumo kwenye viungo vya mwili',
        riskLevelEn: 'Visceral belly fat & joint pressure risk',
        level: 3,
      };
    }
    if (bmi >= 30 && bmi <= 34.9) {
      return {
        ...
        riskLevelSw: 'Hatari ya Shinikizo la Damu & Kisukari',
        riskLevelEn: 'Elevated cardiovascular & metabolic strain',
        level: 4,
      };
    }
    return {
      ...
      riskLevelSw: 'Hatari kubwa ya mafuta kuziba mishipa na uchovu mkubwa',
      riskLevelEn: 'High visceral fat accumulation and arterial strain',
      level: 5,
    };
  }, [bmi]);
  ...
  const prescription = useMemo(() => {
    ...
    if (goal === 'ulcers') {
      ...
      return {
        titleSw: 'Mpango wa Asili wa Vidonda vya Tumbo & Asidi (14-Day Reset)',
        ...
        descEn: 'Alkaline chlorophyll soothes gastric lining inflammation within 15 minutes, paired with pure Spirulina for easy nutrient absorption.',
        timelineSw: 'Mabadiliko yanaonekana ndani ya siku 3–5',
    ...
    if (goal === 'energy') {
      ...
      return {
        ...
        titleEn: 'Peak Male Stamina & Circulation Bundle',
  ...
  const handleConsultWhatsApp = () => {
    ...
    const message = [
      ...
      `📏 *Urefu:* ${heightCm} cm | ⚖️ *Uzito:* ${weightKg} kg`,
      `📊 *Kipimo cha BMI:* ${bmi} (${categoryName})`,
      excessWeight > 0 ? `⚖️ *Uzito wa Kupunguza:* ~${excessWeight} kg` : `✨ *Hali:* Uzito mzuri wa kiafya`,
      `🎯 *Lengo:* ${goalTitle}`,
      `━━━━━━━━━━━━━━━━━━━━`,
      `Habari ${distributor.name}! Nimepima BMI yangu kwenye tovuti na nimepata mapendekezo haya ya bidhaa. Naomba mwongozo wa jinsi ya kuanza dozi hii na jinsi ya kuipokea.`,
    ].join('\n');
  ...
  return (
    ...
    >
      {/* ── HEADER BANNER ── */}
      <div className="bg-gradient-to-r from-emerald-900 via-emerald-800 to-primary-800 p-6 sm:p-8 text-white relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none" />
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1.5">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 backdrop-blur-md rounded-full text-xs font-bold text-emerald-200 border border-white/10">
          ...
          <div className="flex-shrink-0 bg-white/10 backdrop-blur-md p-3.5 rounded-2xl border border-white/10 text-center min-w-[140px]">
            <p className="text-[11px] uppercase tracking-wider text-emerald-200 font-semibold">
              {lang === 'sw' ? 'Alama ya BMI Yako' : 'Your BMI Score'}
            </p>
            <div className="text-3xl sm:text-4xl font-extrabold text-amber-300 my-0.5">{bmi}</div>
            <div className="text-[10px] font-bold text-white px-2 py-0.5 rounded-md bg-white/20 inline-block">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-neutral-50 p-4 rounded-2xl border border-neutral-200/80">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-base font-extrabold text-emerald-700 bg-white px-2.5 py-0.5 rounded-lg border border-neutral-200 shadow-2xs">
                  {weightKg} kg
                </span>
              </div>
              <input
        ...
        <div className="lg:col-span-7 space-y-5">
          ...
          <div className={`p-5 rounded-2xl border ${bmiCategory.color} transition-all`}>
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-white/80 border border-neutral-200/60">
            ...
            <div className="mt-3.5 mb-2">
              <div className="h-3 w-full bg-neutral-200 rounded-full overflow-hidden flex">
                <div className="w-[18%] bg-sky-400" title="Underweight (<18.5)" />
                <div className="w-[30%] bg-emerald-500" title="Normal (18.5-24.9)" />
                <div className="w-[22%] bg-amber-400" title="Overweight (25-29.9)" />
                <div className="w-[18%] bg-orange-500" title="Obesity I (30-34.9)" />
                <div className="w-[12%] bg-rose-600" title="Severe (>35)" />
              </div>
              <div className="flex justify-between text-[9px] text-neutral-500 mt-1 font-semibold">
                <span>18.5</span>
                <span>25.0</span>
                <span>30.0</span>
                <span>35.0+</span>
            ...
            <p className="text-xs mt-2 text-neutral-800 leading-relaxed font-medium">
              💡 <strong>{lang === 'sw' ? 'Tathmini ya Kiafya:' : 'Clinical Assessment:'}</strong>{' '}
              {lang === 'sw' ? bmiCategory.riskLevelSw : bmiCategory.riskLevelEn}
            </p>

            <div className="mt-3 pt-3 border-t border-neutral-200/60 grid grid-cols-2 gap-2 text-xs">
              <div>
          ...
          <div className="p-5 sm:p-6 bg-emerald-950 text-white rounded-3xl border border-emerald-800 shadow-md space-y-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-amber-400 text-emerald-950 text-[10px] font-black uppercase tracking-wider mb-1.5">
            ...
            <div className="space-y-2 pt-2 border-t border-emerald-800/80">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {prescription.items.map((prod) => (
                  <div
                    key={prod.id}
                    className="p-2.5 rounded-xl bg-white/10 border border-white/10 flex items-center gap-2.5"
                  >
                    <img
                      src={prod.image}
                      alt={prod.name.en}
                      className="w-10 h-10 object-contain rounded-lg bg-white/20 p-1 flex-shrink-0"
                    />
            ...
            <div className="p-3.5 bg-white/10 backdrop-blur-md rounded-2xl border border-white/10 flex flex-wrap items-center justify-between gap-3">
              <div>
                <span className="text-[11px] text-emerald-200 line-through block">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 bg-rose-500 text-white font-black text-[10px] rounded-md uppercase">
              ...
              <span className="text-[11px] text-emerald-200 font-medium">
                ⏱️ {lang === 'sw' ? prescription.timelineSw : prescription.timelineEn}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
              <button
                ...
                className="w-full py-3 px-4 rounded-xl bg-amber-400 hover:bg-amber-300 text-emerald-950 font-extrabold text-xs sm:text-sm flex items-center justify-center gap-2 shadow-sm transition-all transform active:scale-95 cursor-pointer"
              >
