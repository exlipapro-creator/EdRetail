...
export const ClientCareCrmPanel: React.FC<ClientCareCrmPanelProps> = ({ lang }) => {
  ...
  return (
    <div className="space-y-6 text-gray-900">
      {/* Overview Banner */}
      <div className="p-4 sm:p-5 panel-inner space-y-2">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="px-2.5 py-0.5 bg-primary-600 text-white font-semibold text-[10px] rounded-md uppercase">
      ...
      <div className="panel-surface p-4 sm:p-5 space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
          <button
            ...
            className={`p-3.5 rounded-md text-left border text-xs transition-all cursor-pointer ${
              selectedScriptType === 'refill'
                ? 'bg-primary-50 border-primary-500 font-semibold text-primary-900 shadow-xs'
      ...
      <div className="space-y-3">
        ...
        {sales.length === 0 ? (
          <div className="p-8 text-center panel-inner text-gray-400 text-xs">
            <Receipt className="w-6 h-6 mx-auto text-gray-300 mb-2" />
          ...
          sales.map((sale) => {
            ...
            return (
              <div
                key={sale.id}
                className="p-4 panel-inner space-y-2.5 hover:border-gray-300 transition-all"
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-gray-900 text-xs sm:text-sm">{sale.customerName}</span>
                      <span className="text-xs text-gray-400 font-mono">({sale.customerPhone || 'Bila Namba'})</span>
                    </div>
                    <div className="text-xs text-gray-500 mt-1">
                      Bidhaa: <strong className="text-gray-900 font-semibold">{sale.productName}</strong> • Tarehe: {new Date(sale.createdAt).toLocaleDateString()}
                    </div>
                  </div>
                  ...
                  <a
                    ...
                    className="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-md text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors self-start sm:self-auto cursor-pointer"
                  >
      ...
      <div className="panel-surface p-4 sm:p-5 space-y-4">
        <div className="flex items-center justify-between">
          <span className="font-semibold text-xs sm:text-sm text-gray-900 flex items-center gap-2">
            <Phone className="w-4 h-4 text-primary-600" />
            <span>{lang === 'sw' ? 'Tuma Ujumbe kwa Mteja Maalum' : 'Quick-Dispatch to Custom Client'}</span>
          </span>
          <span className="text-[10px] bg-primary-50 text-primary-700 border border-primary-200 px-2 py-0.5 rounded-md font-semibold">1-Tap WhatsApp</span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 panel-inner p-3.5">
          <div>
            <label className="block text-[11px] font-semibold text-gray-500 mb-1">{lang === 'sw' ? 'Jina:' : 'Name:'}</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-[11px] font-semibold text-gray-500 mb-1">{lang === 'sw' ? 'Simu WhatsApp:' : 'WhatsApp Phone:'}</label>
            <input
              type="tel"
          ...
          <div>
            <label className="block text-[11px] font-semibold text-gray-500 mb-1">{lang === 'sw' ? 'Bidhaa:' : 'Product:'}</label>
            <input
              type="text"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          <button
            ...
            className="p-2.5 bg-white hover:bg-gray-50 border border-gray-300 text-gray-600 rounded-md text-[11px] font-semibold text-center transition-colors cursor-pointer"
          >
            Siku 3 (Maji)
          </button>
          <button
          ...
          <button
            ...
            className="p-2.5 bg-green-50 hover:bg-green-100 border border-green-200 text-green-700 rounded-md text-[11px] font-semibold text-center transition-colors cursor-pointer"
          >
            Siku 14 (Refill)
          </button>
          <button
      ...
      <div className="panel-surface p-4 sm:p-5 space-y-3">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="px-4 py-2.5 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-md transition-colors cursor-pointer whitespace-nowrap text-xs"
          >
            {lang === 'sw' ? 'Jaribu' : 'Test'}
          </button>
        </div>
        ...
        {simResult && (
          <div className="p-3.5 bg-gray-50 rounded-md border border-gray-200 space-y-1.5">
            <span className="text-[10px] font-semibold uppercase text-primary-700 tracking-wider">
