...
export function ReferralShareButton({ variant = 'default' }: ReferralShareButtonProps) {
  ...
  return (
    <button
      onClick={handleShare}
      className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-stone-900 text-white rounded-xl text-xs font-bold hover:bg-stone-800 active:bg-stone-950 transition-colors outline-none shadow-2xs whitespace-nowrap [-webkit-tap-highlight-color:transparent]"
    >
      <Share2 className="w-3.5 h-3.5" />
      {lang === 'sw' ? 'Shiriki na Rafiki' : 'Share with a Friend'}
    </button>
